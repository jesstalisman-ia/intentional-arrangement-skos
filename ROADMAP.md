...
### Crosswalk — federate taxonomies into a thesaurus (v0.9.0)
- A new **Crosswalk** tab aligns two taxonomies from your workspace with the five SKOS mapping relations (`exactMatch`, `closeMatch`, `broadMatch`, `narrowMatch`, `relatedMatch`). **Auto-match by label** proposes links (identical labels → `exactMatch`, near labels → `closeMatch`); confirm the ones you want or draw your own in the visual view (click source, click target). Mappings are stored on the source concepts, so they flow into every normal RDF export — and you can export the **crosswalk alone** (`crosswalk.ttl`) or the whole **federated thesaurus** (both schemes + mappings) as one interoperable file.
